...
The third model is aligned with the zebra HMM-SSF specification used in
Klappstein, Thomas, and Michelot (2023). It uses the same two-state structure,
the same vegetation and movement covariates, the same time-of-day transition
formula, 25 gamma-generated control steps per observed step, and the same
multi-start fitting strategy: all six stored starting-value sets are fitted,
then the fit with the lowest negative log-likelihood is retained.
...
The quantities shown in both trajectory-level and state-level tables are discrepancy
statistics. The "Observed" column is the discrepancy between each observed statistic
and the model null reference, while simulated values are summaries of the same
discrepancy under Markov simulation from each fitted model. Smaller discrepancies
indicate closer agreement to the model.

In the embedded run, Monte Carlo p-values are based on `n_sims = 99` simulations.
The rank-test resolution is therefore `1 / (99 + 1) = 0.01`, so very small
numeric differences in p-values should be interpreted with that grid in mind.
...
add_discrepancy_aliases <- function(x) {
  if (!"observed_discrepancy" %in% names(x) && "observed_statistic" %in% names(x)) {
    x$observed_discrepancy <- x$observed_statistic
  }
  if (!"simulated_discrepancy_median" %in% names(x) && "simulated_median" %in% names(x)) {
    x$simulated_discrepancy_median <- x$simulated_median
  }
  if (!"simulated_discrepancy_q025" %in% names(x) && "simulated_q025" %in% names(x)) {
    x$simulated_discrepancy_q025 <- x$simulated_q025
  }
  if (!"simulated_discrepancy_q975" %in% names(x) && "simulated_q975" %in% names(x)) {
    x$simulated_discrepancy_q975 <- x$simulated_q975
  }
  if (!"mc_p_value" %in% names(x) && "p_value" %in% names(x)) {
    x$mc_p_value <- x$p_value
  }
  x
}

global_diagnostics <- add_discrepancy_aliases(global_diagnostics)
diagnostics <- add_discrepancy_aliases(diagnostics)

for (column in c("fit_strategy", "n_starts", "selected_start")) {
  if (!column %in% names(model_summary)) {
    model_summary[[column]] <- NA
  }
}
...
knitr::kable(
  ...
  col.names = c(
    ...
    "Simulated discrepancy median",
    "Simulated discrepancy q025",
    "Simulated discrepancy q975",
    "Monte Carlo p-value"
  )
)
```
...
observed_simulated <- rbind(
  data.frame(
    ...
    value = global_diagnostics$observed_discrepancy
  ),
  data.frame(
    model_label = global_diagnostics$model_label,
    diagnostic_label = global_diagnostics$diagnostic_label,
    source = "Simulated discrepancy median",
    value = global_diagnostics$simulated_discrepancy_median
  )
)
...
ggplot(
  ...
  labs(
    ...
    color = NULL,
    shape = NULL,
    title = "Observed and simulated discrepancies",
...
For state diagnostics, the same interpretation rule applies: the reported numbers
are discrepancy summaries. In two-state fits, state occupancy rows can be identical
because the two occupancy proportions must sum to one and the same absolute
discrepancy is reported per state. Step length and residence-time entries are based
on full distributional distances, not raw mean step lengths or mean dwell times.
Observed trajectories are state-decoded with Viterbi, while simulated trajectories
use the simulated latent state sequence, so this is a comparison on different
state-labelling views.

In this embedded run, no candidate model reproduces every global diagnostic.
A model can appear visually closer on one diagnostic and still have a lower Monte
Carlo p-value if its simulated null is more concentrated for that statistic. This is
why discrepancies and p-values should be read together.
...
initial_par_list <- readRDS(file.path(
  hmmssf_repo,
...
To assess memory effects in the regime process more directly, a direct HSMM
extension can be a useful additional simulation study: simulate trajectories from
an HSMM-SSF with non-geometric dwell times, fit a standard HMM-SSF, and check
whether state-level residence-time discrepancies lose power when the memory
structure is misspecified. This is a natural step for evaluating whether a
non-geometric dwell-time model is required.
